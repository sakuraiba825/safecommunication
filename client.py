...
def connectServer():
    ...
    #family: AF_UNIX用于同一台机器上的进程间通信，AF_INET对于IPV4协议的TCP和UDP 。
    #type根据是面向连接的还是非连接分为SOCK_STREAM或SOCK_DGRAM
...
labelPort = tkinter.Label(win, text="服务器port").grid(row=2, column=0)
